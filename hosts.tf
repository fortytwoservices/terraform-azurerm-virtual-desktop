...
resource "azurerm_virtual_machine_extension" "avd-session-host-azuread-join" {
  for_each = { for k, v in local.session_host_vms : k => v if local.session_host_vms.azure_domain_join_type == "azuread" ? true : false }
...
resource "azurerm_virtual_machine_extension" "avd-session-host-aadds-join" {
  for_each = { for k, v in local.session_host_vms : k => v if local.session_host_vms.azure_domain_join_type == "aadds" ? true : false }
