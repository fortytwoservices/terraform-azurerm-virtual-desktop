...
resource "azurerm_virtual_desktop_host_pool" "avd-host_pools" {
  ...
  dynamic "scheduled_agent_updates" {
    ...
    content {
      enabled                   = lookup(scheduled.value[0], "enabled", null)
      timezone                  = lookup(scheduled.value[0], "timezone", null)
      use_session_host_timezone = lookup(scheduled.value[0], "use_session_host_timezone", null)
